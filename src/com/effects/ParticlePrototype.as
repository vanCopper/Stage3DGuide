package com.effects
{
	...
	public class ParticlePrototype
	{
		...
		public function ParticlePrototype(width:Number, height:Number, atlas:TextureAtlas, 
										  animated:Boolean = false, blendSource:String = "sourceAlpha", 
										  blendDestination:String = "oneMinusSourceAlpha") 
		{
			this.width = width;
			this.height = height;
			this.atlas = atlas;
		...
		public function addKey(time:Number, rotation:Number = 0, scaleX:Number = 1, 
							   scaleY:Number = 1, red:Number = 1, green:Number = 1, 
							   blue:Number = 1, alpha:Number = 1):void 
		{
		...
		public function createParticle(effect:ParticleEffect, time:Number, position:Vector3D,
									   rotation:Number = 0, scaleX:Number = 1, scaleY:Number = 1, 
									   alpha:Number = 1, firstFrame:int = 0):void
		{
			var b:int = keysCount - 1;
			if (atlas.diffuse.texture != null && keysCount > 1
				&& time >= timeKeys[0] && time < timeKeys[b]) 
			{
				...
				for (b = 1; b < keysCount; b++) 
				{
					if (time < timeKeys[b]) 
					{
						...
						particle.diffuse = atlas.diffuse.texture;
						particle.opacity = (atlas.opacity != null) ? atlas.opacity.texture : null;
